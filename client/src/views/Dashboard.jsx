...
const Dashboard = () => {
  ...
  return (
    <Container className='d-flex flex-column'>
      {playlists.map((item)=>(
        <div className='d-flex flex-column bg-danger m-3 align-items-center' key={item._id}>
          <h5 className=' text-light m-3'>Playlist Title: <Link to={`/playlist/${item._id}/details`}className='text-light ' style={{textDecorationLine:'underline'}}>{item.title}</Link></h5>
